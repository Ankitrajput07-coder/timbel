...
/**
 * POST /run-sql
 * Accepts a raw SQL INSERT query string generated by the AI,
 * parses it into objects, and inserts them using Supabase JS.
 */
router.post('/run-sql', authMiddleware, async (req, res) => {
  try {
    const { sql, campus_id } = req.body;
    if (!campus_id) return res.status(400).json({ error: 'campus_id is required' });
    if (!sql) return res.status(400).json({ error: 'SQL query is required' });

    // Validate and extract VALUES section
    const valuesMatch = sql.match(/VALUES\s*([\s\S]+);?/i);
    if (!valuesMatch) {
      return res.status(400).json({ error: 'Could not parse SQL. Must contain VALUES keyword.' });
    }
    ...
    // Using a regex that correctly splits only on `),` at the end of a tuple
    const rows = valuesStr.split(/\)(?:\s*,\s*|\s*$)/).filter(r => r.trim().length > 0);
    ...
    for (let rowStr of rows) {
      ...
      // Match string literals ('...') or numbers
      const regex = /'([^']*)'|(\d+)/g;
      let match;
      const values = [];
      ...
      if (values.length >= 12) {
        parsedData.push({
          ...
          start_time: values[5],
          end_time: values[6],
          session_type: values[7],
          class_code: values[8],
          section: values[9],
          program: values[10],
          year: parseInt(values[11], 10),
          campus_id: campus_id
        });
      }
    }
    ...
    if (parsedData.length === 0) {
      return res.status(400).json({ error: 'No valid data rows parsed from SQL.' });
    }
    ...
    res.json({ success: true, rowCount: parsedData.length });
  } catch (err) {
    console.error('SQL Execution Error:', err);
    res.status(500).json({ error: 'SQL parsing/execution failed', details: err.message });
  }
});
