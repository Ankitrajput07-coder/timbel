  ...
  const handleRunSql = async () => {
    ...
    try {
      const res = await fetch('/api/admin/run-sql', {
        ...
        body: JSON.stringify({ sql: rawSql, campus_id: selectedCampus })
      })
      ...
      setUploadStatus('success')
      setUploadMessage(`Successfully executed SQL and inserted ${data.rowCount} slots.`)
      setRawSql('')
    } catch (err) {
      setUploadStatus('error')
      setUploadMessage(err.message)
      ...
      <div className="max-w-3xl mx-auto">
        ...
        {activeTab === 'upload' && (
          <div className="glass-card p-8 flex flex-col gap-6">
            ...
            <div>
              <h3 className="text-xl font-bold font-heading text-text-primary mb-4 flex items-center gap-2">
                <Upload className="w-5 h-5 text-violet-primary" /> Upload from Excel
              </h3>
              <div 
            ...
            <div className="h-px bg-slate-border w-full my-2"></div>

            {/* Raw SQL Section */}
            <div>
              <h3 className="text-xl font-bold font-heading text-text-primary mb-4 flex items-center gap-2">
                <Shield className="w-5 h-5 text-emerald-free" /> Execute Raw SQL
              </h3>
              <p className="text-sm text-text-muted mb-4">Paste the INSERT query generated by the AI to add timetable slots directly.</p>
              
              <textarea
                value={rawSql}
                onChange={e => setRawSql(e.target.value)}
                placeholder="INSERT INTO timetable_slots (building, room, subject, ...) VALUES ..."
                className="w-full h-40 p-4 rounded-xl bg-slate-darker border border-slate-border text-text-primary font-mono text-xs focus:outline-none focus:border-emerald-free focus:ring-1 focus:ring-emerald-free transition-colors"
              />
              
              <div className="mt-4 flex justify-end">
                <button
                  onClick={handleRunSql}
                  disabled={sqlRunning || !rawSql.trim()}
                  className="px-8 py-3 rounded-full bg-emerald-free hover:bg-emerald-free/90 text-white font-medium transition-colors disabled:opacity-50 flex items-center gap-2"
                >
                  {sqlRunning ? (
                    <><Lottie animationData={loaderAnimation} className="w-5 h-5" /> Executing...</>
                  ) : (
                    <>Run SQL Query</>
                  )}
                </button>
